# REVISION BRIEF — {{book}} — Chapter {{NN}} "{{title}}" — edition {{n}} → {{n+1}}
Compiled {{date}} by the orchestrator. **Two pages plus the existing chapter. You are revising a chapter that exists, against what its readers said.** Walls not listed here still bind and are enforced by the editor and verifier.

## 1. Load (five files, in order)
1. `pen-names/fantasyau1.1/SOUL.md`
2. `pen-names/fantasyau1.1/VOICE.md`
3. `chapters/ch{{NN}}-{{slug}}.md` — the chapter as it stands (edition {{n}})
4. `chapters/ch{{NN-1}}-*.md` — the previous chapter, as it stands after its own revision
5. This brief

## 2. The revision card (verbatim)
{{card}}

## 3. The readers, in their own words
