# Fantasy Author A — Movement Pipeline

This is the default production loop for `/fanauthr`. It preserves the original
progression-fantasy/LitRPG voice while letting Fable draft long enough to sustain
voice and narrative momentum.

## Default unit

A writing session drafts one contiguous movement of three to five chapters.
Four is the default. Use three when the movement contains a major reveal or
complicated fight. Use five when the cards are short, sequential, and already
well specified.

Before drafting, the movement needs:

- approved cards for every chapter in the batch;
...
3. **Light close.** Between chapters check only canon, card function, state,
   names, word band, reader standard, and whether the promised scene is actually
   dramatized. Repair a blocker immediately. Defer polish and preference.
4. **Freeze the batch.** File one batch report. Do not start the next movement
   while the current batch is still changing invisibly.
...
10. **Close and advance.** Update canon rulings, ledger, registry, injuries, and
    open threads. Then compile the next movement packet.

## Larger passes

At each act boundary, read the act continuously for thread debt, escalation,
relationship movement, action/humor rhythm, and repeated scene scaffolds. At book
lock, run separate whole-book passes in this order: structural continuity, voice
and character distinction, line edit, then listening proof. A later pass does not
quietly redo an earlier one.
...
pass. These are planning estimates, not quotas.

## Exception lane

Use the original single-chapter edit immediately when a chapter contains a major
canon ruling, irreversible death, complex system-law reveal, or the batch cannot
continue safely without knowing whether the chapter works. This is an exception,
